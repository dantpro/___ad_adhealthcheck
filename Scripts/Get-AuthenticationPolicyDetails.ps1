...
if ($($ADAuthenticationPolicy.count) -ne 0) {
    $ADAuthenticationPolicy | export-csv -Path $OutputPath\Authentication_Policy_Details.csv -NoTypeInformation -Encoding UTF8 -Delimiter ";"
}
...
if ($($ADAuthenticationPolicySilo.count) -ne 0) {
    $ADAuthenticationPolicySilo | export-csv -Path $OutputPath\Authentication_Policy_Details_Silo.csv -NoTypeInformation -Encoding UTF8 -Delimiter ";"
}
